...
3. 意図どおりなら PR を作成（または更新）し、ラベルを付ける
   ...
   ```
4. CI が `recordRoborazziDebug` を実行し、変更があれば「スナップショットのベースラインを更新 (CI)」としてコミット、ラベルを外し、CI を再実行する。結果は PR コメントに投稿される
...
- `update-snapshots` ワークフローは `GITHUB_TOKEN` で push するため、その push 自体は CI をトリガーしない。ワークフロー内で `gh workflow run ci.yml` により明示的に再実行している
